...
class Ui_MainWindow(object):
    ...
    try:
        ...
        for cluster_id in range(num_clusters):
            centroid = X[data['Cluster'] == cluster_id].mean()
            centroids.append(centroid)

        # Write cluster assignments and centroids to CSV
        cluster_centroids = pd.DataFrame(centroids, columns=['Frequency (Hz)', 'Angle of Arrival (degrees)'])
        cluster_centroids['Cluster'] = range(num_clusters)  # Add cluster IDs to centroids

        # Save cluster and centroid info to CSV
        cluster_info_file, _ = QFileDialog.getSaveFileName(None, "Save Cluster Info CSV", "", "CSV Files (*.csv)")
        if cluster_info_file:
            data.to_csv(cluster_info_file, index=False)
            cluster_centroids.to_csv(cluster_info_file, mode='a', index=False, header=False)  # Append centroids

        # Generate linkage matrix and plot dendrogram
        linkage_matrix = hierarchy.linkage(X, method='ward')
        plt.figure(figsize=(10, 6))
        dendrogram = hierarchy.dendrogram(linkage_matrix, truncate_mode='level', p=num_clusters)
        ...
        save_dendrogram_file, _ = QFileDialog.getSaveFileName(None, "Save Dendrogram Image", "", "PNG Files (*.png)")
