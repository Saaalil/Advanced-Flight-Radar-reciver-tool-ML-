...
class ClusteringApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def dbscan_clustering(self, file_path, eps_angle, min_samples_angle, eps_frequency, min_samples_frequency, save_path):
        try:
            ...
            clustered_df.to_csv(save_path, index=False)   # Adjust the file path as needed
        
            # Plotting - DBSCAN Clustering on Frequency
            for idx, df_cluster in enumerate(clustered_dfs):
                plt.figure(figsize=(10, 8))
                scatter_angle = plt.scatter(df['Angle of Arrival (degrees)'], df['Frequency (Hz)'], c=df['Cluster_Angle'], cmap='viridis', s=50)
                plt.title(f'DBSCAN Clustering (Frequency) - Cluster {idx+1}')
                plt.xlabel('Frequency (Hz)')
                plt.ylabel('Angle of Arrival (degrees)')
                plt.colorbar(label='Cluster')
                plt.grid(True)
                legend1 = plt.legend(*scatter_angle.legend_elements(),
                             title="Clusters", 
                             loc="upper right")
...
def angle_based_clustering(self, df, threshold_angle):
        clusters = []
        ...
        for i in range(len(df)):
            ...
            if len(clusters) == 0:
                # If no clusters exist, start a new cluster with the current row
                clusters.append(row)
                continue
            ...
            for j in range(len(clusters)):
                cluster = clusters[j]
                mean_point = self.calculate_mean(cluster)
                if abs(row['Angle of Arrival (degrees)'].values[0] - mean_point['Angle of Arrival (degrees)']) < threshold_angle:
                    # If the row fits within the angle threshold of an existing cluster, add it to that cluster
                    clusters[j] = pd.concat([cluster, row])
                    added_to_cluster = True
                    break

            if not added_to_cluster:
                # If the row did not fit into any existing cluster, start a new cluster with the current row
                clusters.append(row)

        return clusters

def frequency_based_clustering(self, df, threshold_frequency):
            clusters = []
...
def plot_clusters(self, angle_clusters, frequency_clusters, threshold_angle, threshold_frequency):
...
def centroid_clustering(self, file_path, threshold_angle, threshold_frequency):
        try:
            df = self.read_csv(file_path)

            # Step 1: Cluster based on Angle of Arrival (AoA)
            angle_clusters = self.angle_based_clustering(df, threshold_angle)

            # Step 2: For each AoA cluster, cluster based on Frequency
            all_frequency_clusters = []
            for angle_cluster_df in angle_clusters:
                frequency_clusters = self.frequency_based_clustering(angle_cluster_df, threshold_frequency)
                all_frequency_clusters.extend(frequency_clusters)

            # Prepare data for saving
            clustered_data = []
            for freq_cluster in all_frequency_clusters:
                mean_point = self.calculate_mean(freq_cluster)
                clustered_data.append(mean_point)

            clustered_df = pd.DataFrame(clustered_data)

            # Plot the clusters
            self.plot_clusters(angle_clusters, all_frequency_clusters, threshold_angle, threshold_frequency)

            # Save the clustered data to files
            self.save_clusters_to_files(clustered_df, threshold_angle, threshold_frequency)
